using System.Text;

namespace Leda.Lang;

/// <summary>
/// Given an AST, emits its corresponding Lua code as a string.
/// </summary>
public class Emitter
{
    private readonly StringBuilder builder = new();
    private readonly char indentChar = ' ';
    private readonly int indentSize = 2;

    private static readonly Dictionary<char, string> EscapeChars = new()
    {
        { '\0', @"\0" },
        { '\a', @"\a" },
        { '\b', @"\b" },
        { '\f', @"\f" },
        { '\r', @"\r" },
        { '\t', @"\t" },
        { '\v', @"\v" },
        { '\\', @"\\" },
        { '\"', "\\\"" },
        { '\n', @"\n" },
    };

    private static bool IsSimpleKey(string name)
    {
        return name.Length > 0 && !char.IsDigit(name[0]) && name.All(Lexer.IsNameChar);
    }

    private static bool AreBinaryParensNeeded(Tree.Expression.Binary outer, Tree.Expression inner)
    {
        return inner is Tree.Expression.Binary innerBinary &&
               Token.Precedence(innerBinary.Operator) < Token.Precedence(outer.Operator);
    }

    private Emitter() { }

    private void Emit(string str)
    {
        builder.Append(str);
    }

    private void Emit(char c)
    {
        builder.Append(c);
    }

    private void Emit(char c, int count)
    {
        builder.Append(c, count);
    }

    private void EmitIndent(int level)
    {
        Emit(indentChar, indentSize * level);
    }

    private void EmitDeclarationList(List<Tree.Declaration> declarations)
    {
        for (var i = 0; i < declarations.Count; i++)
        {
            Emit(declarations[i].Name.Value);
            if (i < declarations.Count - 1)
            {
                Emit(", ");
            }
        }
    }

    private void EmitFunctionBody(Tree.Expression.Function function, int indent)
    {
        Emit('(');
        EmitDeclarationList(function.Type.Parameters);
        Emit(")\n");
        EmitBlock(function.Chunk, indent + 1);
        EmitIndent(indent);
        Emit("end");
    }

    private void EmitExpression(Tree.Expression expression, int indent, bool parenthesize = false)
    {
        if (parenthesize)
        {
            Emit('(');
        }

        if (expression is Tree.Expression.Name name)
        {
            Emit(name.Value);
        }
        else if (expression is Tree.Expression.Number number)
        {
            Emit(number.Value);
        }
        else if (expression is Tree.Expression.True)
        {
            Emit("true");
        }
        else if (expression is Tree.Expression.False)
        {
            Emit("false");
        }
        else if (expression is Tree.Expression.Nil)
        {
            Emit("nil");
        }
        else if (expression is Tree.Expression.Vararg)
        {
            Emit("...");
        }
        else if (expression is Tree.Expression.LongString longString && longString.Level > -1)
        {
            Emit('[');
            Emit('=', longString.Level);
            Emit('[');
            Emit(longString.Value);
            Emit(']');
            Emit('=', longString.Level);
            Emit(']');
        }
        else if (expression is Tree.Expression.String str)
        {
            Emit('"');
            foreach (var c in str.Value)
            {
                if (EscapeChars.TryGetValue(c, out var escaped))
                {
                    Emit(escaped);
                }
                else
                {
                    Emit(c);
                }
            }

            Emit('"');
        }
        else if (expression is Tree.Expression.Table table)
        {
            Emit('{');
            var lastNumberIndex = 1;
            for (var i = 0; i < table.Fields.Count; i++)
            {
                var field = table.Fields[i];
                // ReSharper disable once CompareOfFloatsByEqualityOperator
                if (field.Key is Tree.Expression.Number numberKey && numberKey.NumberValue == lastNumberIndex)
                {
                    EmitExpression(field.Value, indent);
                    lastNumberIndex++;
                }
                else if (field.Key is Tree.Expression.String stringKey && IsSimpleKey(stringKey.Value))
                {
                    Emit(stringKey.Value);
                    Emit(" = ");
                    EmitExpression(field.Value, indent);
                }
                else
                {
                    Emit('[');
                    EmitExpression(field.Key, indent);
                    Emit("] = ");
                    EmitExpression(field.Value, indent);
                }

                if (i < table.Fields.Count - 1)
                {
                    Emit(", ");
                }
            }

            Emit('}');
        }
        else if (expression is Tree.Expression.Function function)
        {
            Emit("function");
            EmitFunctionBody(function, indent);
        }
        else if (expression is Tree.Expression.Access access)
        {
            EmitPrefixExpression(access.Target, false, indent);
            if (access.Key is Tree.Expression.String stringKey && IsSimpleKey(stringKey.Value))
            {
                Emit('.');
                Emit(stringKey.Value);
            }
            else
            {
                Emit('[');
                EmitExpression(access.Key, indent);
                Emit(']');
            }
        }
        else if (expression is Tree.Expression.Call call)
        {
            EmitCall(call, false, indent);
        }
        else if (expression is Tree.Expression.MethodCall methodCall)
        {
            EmitMethodCall(methodCall, false, indent);
        }
        else if (expression is Tree.Expression.Binary binary)
        {
            EmitExpression(binary.Left, indent, AreBinaryParensNeeded(binary, binary.Left));

            Emit(' ');
            Emit(binary.Operator.Value);
            Emit(' ');

            EmitExpression(binary.Right, indent, AreBinaryParensNeeded(binary, binary.Right));
        }
        else if (expression is Tree.Expression.Unary unary)
        {
            Emit(unary.Operator.Value);
            if (unary.Operator.Kind == TokenKind.Not)
            {
                Emit(' ');
            }

            EmitExpression(unary.Expression, indent, unary.Expression is Tree.Expression.Binary);
        }
        else if (expression is not Tree.Expression.Error)
        {
            throw new Exception();
        }

        if (parenthesize)
        {
            Emit(')');
        }
    }

    private void EmitExpressionList(List<Tree.Expression> values, int indent)
    {
        for (var i = 0; i < values.Count; i++)
        {
            EmitExpression(values[i], indent);
            if (i < values.Count - 1)
            {
                Emit(", ");
            }
        }
    }

    private void EmitPrefixExpression(Tree.Expression expression, bool isStatement, int indent)
    {
        var parensNeeded = expression is not (Tree.Expression.Call or Tree.Expression.Access or Tree.Expression.Name);

        if (parensNeeded && isStatement)
        {
            // Prevent ambiguities if the line starts with a '('
            Emit(';');
        }

        EmitExpression(expression, indent, parensNeeded);
    }

    private void EmitCall(Tree.Expression.Call call, bool isStatement, int indent)
    {
        EmitPrefixExpression(call.Target, isStatement, indent);
        Emit('(');
        EmitExpressionList(call.Arguments, indent);
        Emit(')');
    }

    private void EmitMethodCall(Tree.Expression.MethodCall call, bool isStatement, int indent)
    {
        EmitPrefixExpression(call.Target, isStatement, indent);
        Emit(':');
        Emit(call.FuncName.Value);
        Emit('(');
        EmitExpressionList(call.Arguments, indent);
        Emit(')');
    }

    private void EmitStatement(Tree.Statement statement, int indent)
    {
        EmitIndent(indent);

        if (statement is Tree.Statement.LocalDeclaration localDeclaration)
        {
            Emit("local ");
            EmitDeclarationList(localDeclaration.Declarations);
            if (localDeclaration.Values.Count > 0)
            {
                Emit(" = ");
                EmitExpressionList(localDeclaration.Values, indent);
            }
        }
        else if (statement is Tree.Statement.LocalFunctionDeclaration functionDeclaration)
        {
            Emit("local function ");
            Emit(functionDeclaration.Name.Value);
            EmitFunctionBody(functionDeclaration.Function, indent);
        }
        else if (statement is Tree.Statement.GlobalDeclaration globalDeclaration)
        {
            if (globalDeclaration.Values.Count > 0)
            {
                EmitDeclarationList(globalDeclaration.Declarations);
                Emit(" = ");
                EmitExpressionList(globalDeclaration.Values, indent);
            }
        }
        else if (statement is Tree.Statement.Call callStmt)
        {
            EmitCall(callStmt.CallExpr, true, indent);
        }
        else if (statement is Tree.Statement.MethodCall methodCallStmt)
        {
            EmitMethodCall(methodCallStmt.CallExpr, true, indent);
        }
        else if (statement is Tree.Statement.Return returnStatement)
        {
            Emit("return");
            if (returnStatement.Values.Count > 0)
            {
                Emit(" ");
                EmitExpressionList(returnStatement.Values, indent);
            }
        }
        else if (statement is Tree.Statement.Do doBlock)
        {
            Emit("do\n");
            EmitBlock(doBlock.Body, indent + 1);
            EmitIndent(indent);
            Emit("end");
        }
        else if (statement is Tree.Statement.If ifStatement)
        {
            Emit("if ");
            EmitExpression(ifStatement.Primary.Condition, indent);
            Emit(" then\n");
            EmitBlock(ifStatement.Primary.Body, indent + 1);
            EmitIndent(indent);

            foreach (var branch in ifStatement.ElseIfs)
            {
                Emit("elseif ");
                EmitExpression(branch.Condition, indent);
                Emit(" then\n");
                EmitBlock(branch.Body, indent + 1);
                EmitIndent(indent);
            }

            if (ifStatement.ElseBody != null)
            {
                Emit("else\n");
                EmitBlock(ifStatement.ElseBody, indent + 1);
                EmitIndent(indent);
            }

            Emit("end");
        }
        else if (statement is Tree.Statement.NumericalFor numericalFor)
        {
            Emit("for ");
            Emit(numericalFor.Counter.Value);
            Emit(" = ");
            EmitExpression(numericalFor.Start, indent);
            Emit(", ");
            EmitExpression(numericalFor.Limit, indent);
            if (numericalFor.Step != null)
            {
                Emit(", ");
                EmitExpression(numericalFor.Step, indent);
            }

            Emit(" do\n");
            EmitBlock(numericalFor.Body, indent + 1);
            EmitIndent(indent);
            Emit("end");
        }
        else if (statement is Tree.Statement.IteratorFor iteratorFor)
        {
            Emit("for ");
            EmitDeclarationList(iteratorFor.Declarations);
            Emit(" in ");
            EmitExpression(iteratorFor.Iterator, indent);
            Emit(" do\n");
            EmitBlock(iteratorFor.Body, indent + 1);
            EmitIndent(indent);
            Emit("end");
        }
        else if (statement is Tree.Statement.While whileLoop)
        {
            Emit("while ");
            EmitExpression(whileLoop.Condition, indent);
            Emit(" do\n");
            EmitBlock(whileLoop.Body, indent + 1);
            EmitIndent(indent);
            Emit("end");
        }
        else if (statement is Tree.Statement.RepeatUntil repeatUntil)
        {
            Emit("repeat\n");
            EmitBlock(repeatUntil.Body, indent + 1);
            EmitIndent(indent);
            Emit("until ");
            EmitExpression(repeatUntil.Condition, indent);
        }
        else if (statement is Tree.Statement.Assignment assignment)
        {
            EmitExpressionList(assignment.Targets, indent);
            Emit(" = ");
            EmitExpressionList(assignment.Values, indent);
        }
        else if (statement is Tree.Statement.Break)
        {
            Emit("break");
        }
        else if (statement is Tree.Statement.LabelDefinition labelDefinition)
        {
            Emit("::");
            Emit(labelDefinition.Name.Value);
            Emit("::");
        }
        else if (statement is Tree.Statement.Goto @goto)
        {
            Emit("goto ");
            Emit(@goto.Name.Value);
        }
        else if (statement is not Tree.Statement.Error)
        {
            throw new ArgumentOutOfRangeException(nameof(statement));
        }

        Emit('\n');
    }

    private void EmitBlock(Tree.Block block, int indent)
    {
        foreach (var statement in block.Statements)
        {
            EmitStatement(statement, indent);
        }
    }

    public static string Emit(Tree.Block block)
    {
        var emitter = new Emitter();
        emitter.EmitBlock(block, 0);
        return emitter.builder.ToString();
    }
}